...
class MovieDetailsPage extends Component {
  ...
  render() {
    const { poster, title, userScore, overview, genres, cast, reviews } = this.state;
    const { match } = this.props;
    return (
      <div className="detailsContainer">
        <button className="returnButton" type="button" onClick={this.handleExit}>
          Return
        </button>
        <div className="detailsBox">
          <div className="posterBox">
            {poster && <img src={`https://image.tmdb.org/t/p/w300${poster}`} alt={title} />}
          </div>
          <div>
